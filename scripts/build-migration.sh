...
./node_node_modules/.bin/tsc -t es2017 -module CommonJS -outDir ./dist-migrations/config ./src/config/database.ts

for filename in ./src/db/migrations/*.ts; do
    ./node_node_modules/.bin/tsc -t es2017 -module CommonJS -outDir ./dist-migrations/db/migrations $filename
done
echo ""
